...
try {
    ...
    switch (action) {
        case "records": {
            ...
            //LogEvent('mw_logger', "result sql: " + ssql);
            ...
            // выполнит sql-запрос
            var result = execSql(ssql, sServerName, param.sFields);
    ...
    LogEvent('mw_logger', "end");
}
catch (err) {
    LogEvent('mw_logger', "ERROR: " + err);
    Response.Write(tools.object_to_text({
        success: false,
        error: String(err),
    }, 'json'))
